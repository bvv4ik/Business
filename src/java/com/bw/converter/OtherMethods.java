        ...
        for (int i = 0; i <= arrName.size() - 1; i++) {
            ...
            if (arrType.get(i).lastIndexOf("Р") != -1) // если это район
            {
                ...
                for (int i2 = i; i2 <= /*arrName.size()-1*/ i + 55; i2++) { // поиск с найденного места - всего 35 след. позций
                    ...
                    {
                        if (arrName.get(i2).contains(" " + sNamePolis) // совпадают 4 букв названия Района
...
// Исправляем ошибки в названиях
                if (arrName.get(i).contains("КУНИЦЬКА/С.КУНИЧНЕ")) {
                    arrName.set(i, "КУНИЦЬКА/С.КУНИЧЕ");
                }
                if (arrName.get(i).contains("МЕЖИСИТІВСЬКА/С.МЕЖИСИТІ")) {
                    arrName.set(i, "МЕЖИСИТІВСЬКА/С.МЕЖИСИТЬ");
                }
                if (arrName.get(i).contains("МИКІЛЬСЬКА/С.МІКІЛЬСЬКЕ-НА-ДНІПРІ")) {
                    arrName.set(i, "МИКІЛЬСЬКА/С.МИКІЛЬСЬКЕ-НА-ДНІПРІ");
                }
                if (arrName.get(i).contains("ОСТРІЙКІВСЬКА/С.ОСТРІЙКІ")) {
                    arrName.set(i, "ОСТРІЙКІВСЬКА/С.ОСТРІЙКИ");
                }
                if (arrName.get(i).contains("ПЕРЕЯСЛАВСЬКА/С.ПЕРЕЯСЛАВСКЕ")) {
                    arrName.set(i, "ПЕРЕЯСЛАВСЬКА/С.ПЕРЕЯСЛАВСЬКЕ");
                }
                if (arrName.get(i).contains("ПОВІТНЕНСЬКА/С.ПОВІТНЕ")) {
                    arrName.set(i, "ПОВІТНЕНСЬКА/С.ПОВІТНО");
                }
                if (arrName.get(i).contains("СВИТАЗІВСЬКА/С.СВИТЯЗІВ")) {
                    arrName.set(i, "СВИТАЗІВСЬКА/С.СВИТАЗІВ");
                }
                if (arrName.get(i).contains("БАРАТІВСЬКА/С.БРАТІВКА")) {
                    arrName.set(i, "БАРАТІВСЬКА/С.БАРАТІВКА");
                }
                if (arrName.get(i).contains("ВОЗНЕСЕНСЬКА-ДРУГА/С.ВОЗНЕСЕНКА-ДРУГА")) {
                    arrName.set(i, "ВОЗНЕСЕНСЬКА ДРУГА/С.ВОЗНЕСЕНКА ДРУГА");
                }
                if (arrName.get(i).contains("ТАВЕРІВСЬКА/С ТАВЕРІВКА")) {
                    arrName.set(i, "ТАВЕРІВСЬКА/С.ТАВЕРІВКА");
                }
                if (arrName.get(i).contains("КАЛІЇВСЬКА/С.КАЛІІВКА")) {
                    arrName.set(i, "КАЛІЇВСЬКА/С.КАЛІЇВКА");
                }
                if (arrName.get(i).contains("АРКАДІВСЬКА/С.АРАКАДІВКА")) {
                    arrName.set(i, "АРКАДІВСЬКА/С.АРКАДІВКА");
                }
                if (arrName.get(i).contains("ОЛЕКСІЇВСЬКА/С.ОЛЕКСІІВКА")) {
                    arrName.set(i, "ОЛЕКСІЇВСЬКА/С.ОЛЕКСІЇВКА");
                }
                if (arrName.get(i).contains("АНТОНОВЕЦЬКА/С.АНТОНОВЦІ")) {
                    arrName.set(i, "АНТОНОВЕЦЬКА/С.АНТОНІВЦІ");
                }
                if (arrName.get(i).contains("БУБНІВСЬКО-СЛОБІДСЬКА/С.БУДНІВСЬКА СЛОБІДКА")) {
                    arrName.set(i, "БУБНІВСЬКО-СЛОБІДСЬКА/С.БУБНІВСЬКА СЛОБІДКА");
                }
                if (arrName.get(i).contains("БАКАЇВСЬКА/С.БАКАІВКА")) {
                    arrName.set(i, "БАКАЇВСЬКА/С.БАКАЇВКА");
                }
                if (arrName.get(i).contains("БІЛОШИЦЬКО-СЛОБІДСЬКА/С.БІЛЬШИЦЬКА СЛОБОДА")) {
                    arrName.set(i, "БІЛОШИЦЬКО-СЛОБІДСЬКА/С.БІЛОШИЦЬКА СЛОБОДА");
                }
                if (arrName.get(i).contains("СТАРОГУТКІВСЬКА/С.СТАРА ГУТА")) {
                    arrName.set(i, "СТАРОГУТКІВСЬКА/С.СТАРА ГУТКА");
                }

                if (arrName.get(i).equals("М.КИЇВ")) {
                    arrName.set(i, "КИЇВ");
                }
                if (arrName.get(i).contains("М.СЕВАСТОПОЛЬ")) {
                    arrName.set(i, "СЕВАСТОПОЛЬ");
                }

                if (arrName.get(i).equals("РАЙОНИ М. КИЇВ")) {
                    arrName.set(i, "РАЙОНИ М.КИЇВ");
                }
                if (arrName.get(i).equals("ЧУДНІВСЬКИЙ РАЙОН/М ЧУДНІВ")) {
                    arrName.set(i, "ЧУДНІВСЬКИЙ РАЙОН/М.ЧУДНІВ");
                }
                if (arrName.get(i).equals("ТАВЕРІВСЬКА/С ТАВЕРІВКА")) {
                    arrName.set(i, "ТАВЕРІВСЬКА/С.ТАВЕРІВКА");
                }
                if (arrName.get(i).equals("САМІЙЛІВСЬКА/С САМІЙЛІВКА")) {
                    arrName.set(i, "САМІЙЛІВСЬКА/С.САМІЙЛІВКА");
                }
                if (arrName.get(i).equals("ПОЛІСЬКИЙ РАЙОН/СМТ.КРАСЯТИЧІ")) {
                    arrName.set(i, "ПОЛІСЬКИЙ РАЙОН/СМТ КРАСЯТИЧІ");
                }

                if (arrName.get(i).contains("ОРДЖОНІКІДЗЕВСЬКА/С.МИКОЛАЇВКА")) {
                    arrName.set(i, "ОРДЖОНІКІДЗЕВСЬКА/C.ОРДЖОНІКІДЗЕ");
                }


                if (arrName.get(i).contains("ВЕЛИКОБІЛОЗЕРСЬКИЙ РАЙОН/С.ВЕЛИКА БІЛОЗЕРКА")) {
                    arrName.set(i, "ВЕЛИКОБІЛОЗЕРСЬКИЙ РАЙОН/С.ВЕЛИКА БІЛОЗЕРКА(ЧАСТИНА 1 СЕЛА)");
                }
                if (arrName.get(i).contains("ВЕЛИКОБІЛОЗЕРСЬКА/С.ВЕЛИКА БІЛОЗЕРКА")) {
                    arrName.set(i, "ВЕЛИКОБІЛОЗЕРСЬКА/С.ВЕЛИКА БІЛОЗЕРКА(ЧАСТИНА 1 СЕЛА)");
                }

                if (arrName.get(i).contains("НОВОПЕТРІВСЬКА/С.ВЕЛИКА БІЛОЗЕРКА")) {
                    arrName.set(i, "НОВОПЕТРІВСЬКА/С.ВЕЛИКА БІЛОЗЕРКА(ЧАСТИНА 2 СЕЛА)");
                }

                if (arrName.get(i).contains("ТРУДОВА/С.ВЕЛИКА БІЛОЗЕРКА")) {
                    arrName.set(i, "ТРУДОВА/С.ВЕЛИКА БІЛОЗЕРКА(ЧАСТИНА 3 СЕЛА)");
                }

                if (arrName.get(i).contains("ЧЕРВОНА/С.ВЕЛИКА БІЛОЗЕРКА")) {
                    arrName.set(i, "ЧЕРВОНА/С.ВЕЛИКА БІЛОЗЕРКА(ЧАСТИНА 4 СЕЛА)");
                }

                if (arrName.get(i).contains("СЕРЕДИНО-БУДСЬКИЙ РАЙОН/М.СЕРЕДИНО-БУДА")) {
                    arrName.set(i, "СЕРЕДИНО-БУДСЬКИЙ РАЙОН/М.СЕРЕДИНА-БУДА");
                }

                if (arrName.get(i).contains("СЕЛИЩА МІСЬКОГО ТИПУ, ПІДПОРЯДКОВАНІ ГОРНЯЦЬКІЙ РАЙРАДІ М.МАКІЇВКИ")) {
                    arrName.set(i, "СЕЛИЩА МІСЬКОГО ТИПУ, ПІДПОРЯДКОВАНІ ГІРНЯЦЬКІЙ РАЙРАДІ М.МАКІЇВКИ");
                }
...
//                        
                ...
                if (!arrName.get(i).contains("///")) { // Если строка не содержит ссылки значит...
                    for (int i3 = 0; i3 <= arrName.size() - 1; i3++) {
                        if ((arrName.get(i3).equals(sNamePolis)) // если область совпадает
    ...
    public static ArrayList ClearGarbage(ArrayList<String> arrInput) {
        ...
        for (int i = 0; i <= arrInput.size() - 1; i++) {
            ...
            if (arrInput.get(i).contains(" РАЙОН")
                    & !arrInput.get(i).contains("РАЙОННОГО")) {                       //если есть такая фраза
